...
import tn.esprit.tache.entities.Department;
import tn.esprit.tache.entities.User;
import tn.esprit.tache.entities.UserRole;
import tn.esprit.tache.entities.UserStatus;
...
public class ServiceUsers implements IServiceu<User> {
    ...
    public void ajouter(User user) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            preparedStatement.setDate(8, new Date(embaucheDate.getTime()));
    ...
    public void modifier(User user) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            preparedStatement.setString(6, user.getIyedPositionUser());
            preparedStatement.setDouble(7, user.getIyedSalaireUser());
            preparedStatement.setDate(8, new Date(user.getIyedDateEmbaucheUser().getTime()));
            preparedStatement.setString(9, user.getIyedStatutUser().name());
            preparedStatement.setObject(10, (user.getIyedDepartment() != null) ? user.getIyedDepartment().getIyedIdDep() : null);
    ...
    public User getUById(int id) {
        String query = "SELECT u.*, d.iyedNomDep, d.iyedIdDep FROM user u LEFT JOIN department d ON u.iyedIdDepUser = d.iyedIdDep WHERE u.iyedIdUser = ?";
        try (PreparedStatement ps = connection.prepareStatement(query)) {
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            ...
            e.printStackTrace(); // Handle exception (log, rethrow, return null, etc.)
        }
    ...
    public Map<Integer, String> getUsersNames() {
        ...
        String sql = "SELECT iyedIdUser, iyedNomUser FROM User";
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                employeMap.put(rs.getInt("iyedIdUser"), rs.getString("iyedNomUser"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
