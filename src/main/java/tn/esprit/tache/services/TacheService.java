...
public class TacheService implements ITacheService {
    ...
    public Map<Integer, String> getUnassignedTacheNames() {
        // Fetch tasks that are NOT assigned
        Map<Integer, String> allTasks = getTacheNames();
        Map<Integer, String> unassignedTasks = new HashMap<>();

        for (Map.Entry<Integer, String> entry : allTasks.entrySet()) {
            if (!isTacheAssigned(entry.getKey())) { // Check if task is assigned
                unassignedTasks.put(entry.getKey(), entry.getValue());
            }
        }
        return unassignedTasks;
    }

    private boolean isTacheAssigned(int tacheId) {
        // Query the database to check if the task is already assigned
        String query = "SELECT COUNT(*) FROM affectation WHERE id_tache = ?";
        try (Connection conn = DBConnection.getInstance().getConnection(); // ✅ Correct usage

             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, tacheId);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) > 0; // If count > 0, task is assigned
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
